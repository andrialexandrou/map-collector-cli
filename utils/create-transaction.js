...
getPeriodName.test = () => {
  console.assert(
    getPeriodName('January', '2019') === '2019-01-01',
    'should be 2019-01-01'
  );
  console.assert(
    getPeriodName('August', '2019') === '2019-08-01',
    'should be 2019-08-31'
  );
  console.assert(
    getPeriodName('Annual', '2019') === undefined,
    'Annual should return undefined'
  );
};
...
createTransaction.test = function() {
  ...
  const outputEmp = createTransaction(
    'employment_data',
    val => multiply(getEmploymentValue(val), 1000),
    exampleReturn
  );
  console.assert(
    isEqual(outputEmp, [
    ...
    'should create employment transaction correctly'
  );

  const outputRecov = createTransaction(
    'recovery_data',
    (val, id) => calculateRecoveryPercentage(id, getRecoveryValue(val)),
    exampleReturn
  );
  console.assert(
    isEqual(outputRecov, [
    ...
    'should create recovery transaction correctly'
  );
};
...
function createBigInsert(tableName, transformFunction, row) {
  ...
  const valuesToAdd = row.data
    .map(datum => {
      ...
      if (value === 'NaN') {
        throw new Error(
          'Could not calculate a value for ' +
            id +
            ', period ' +
            period +
            '\n' +
...
createBigInsert.test = function() {
  ...
  const outputEmp = createBigInsert(
    'employment_data',
    val => multiply(getEmploymentValue(val), 1000),
    exampleReturn
  );
  ...
  const outputRecov = createBigInsert(
    'recovery_data',
    (val, id) => calculateRecoveryPercentage(id, getRecoveryValue(val)),
    exampleReturn
  );
